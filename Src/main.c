...
int main(void)
{
  ...
  // Testing the crystal: TODO remove these lines
  RCC->CR |= RCC_CR_HSEON;
	while(!(RCC->CR & RCC_CR_HSERDY))
		__ISB();
	__DSB(); // set breakpoint here, is it reached?
  ...
  // For testing: attempt to repeatedly message UART
  // Before the OS is initialized.
  // char *data = "Hello from main!\n";
  // while(1) {
  //   // Toggle an LED
...
void StartDefaultTask(void *argument)
{
  ...
  // TODO: replace all the returns in this function with some error handling.
  if(lSocket < 0) return;
...
void Error_Handler(void)
{
  ...
  while(1); // Spin to preserve stack for debugging
